# Hacking ani-cli
Ani-cli is set up to scrape one platform - currently hianime.at. Supporting multiple sources at a time would require more changes than we (the maintainers) find worth doing, for this reason any feature request asking for a new site is rejected.
...
This might be a series overview page (like with hianime.at) or there might not be such, but the episode pages have links to all episodes.

You'll have to edit the `hianime_episodes` function that downloads this list of urls.
...
In case you can get them without loading the episode page, replace from the `get the embed urls...` part of the code to the removal of the cache dir with a single call to `get_video_link` and load its output into `links`.
...
This is done in the script with the `hianime_m3u8` function.
...
The output of the `hianime_m3u8` function needs to be concatenated into the `links` variable - with a single call if there's only one source, or with the asynchronous mode if there are more.
From here the `get_video_link` function will continue with quality selection which you need not to alter.
